...
class SimpleEdgeNetPredictor:
    
    def __init__(self, model_path: str, target_scales: int = 16, target_features: int = 20):
        """
    ...
    def _load_model(self):
        """Load the trained SimpleEdgeNet model"""
        try:
            self.model = tf.keras.models.load_model(self.model_path)
            print(f"Successfully loaded model from {self.model_path}")
            print(f"Model expects flattened input of size {self.target_scales * self.target_features}")
            print(f"({self.target_scales} scales × {self.target_features} features)")
    ...
    def _prepare_model_input(self, ssm_data: np.ndarray) -> np.ndarray:
        """
        Prepare SSM data for model input format (flattened)
        
        Args:
            ssm_data: SSM data of shape (n_points, n_scales, n_features)
            
        Returns:
            Flattened data of shape (n_points, n_scales * n_features)
        """
        # Flatten the data for simple neural network
        # Shape: (n_points, n_scales, n_features) -> (n_points, n_scales * n_features)
        flattened_data = ssm_data.reshape(ssm_data.shape[0], -1)
        
        expected_input_size = self.target_scales * self.target_features
        if flattened_data.shape[1] != expected_input_size:
            raise ValueError(f"Flattened input size {flattened_data.shape[1]} doesn't match expected size {expected_input_size}")
    ...
    def predict_batch(self, ssm_data: np.ndarray, batch_size: int = 1024) -> np.ndarray:
        ...
        Returns:
            Predictions array of shape (n_points, 1)
        """
...
def main():
    parser = argparse.ArgumentParser(description="SimpleEdgeNet Edge Detection Prediction")
    parser.add_argument("--model", required=True, help="Path to trained SimpleEdgeNet model (.h5 file)")
    ...
    print("Initializing SimpleEdgeNet predictor...")
    predictor = SimpleEdgeNetPredictor(
